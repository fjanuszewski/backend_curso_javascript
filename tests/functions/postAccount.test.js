...
let app;
describe.only('post Account', () => {
  beforeAll(async () => {
    //avoid throwing because of local testing
    process.env.AWS_XRAY_CONTEXT_MISSING = 'LOG_ERROR';
    process.env.LOCAL = 'true';
    // @ts-ignore
    AWSXRay.captureFunc.mockResolvedValue('done');
    app = require('../../src/functions/postAccount/postAccount');
  });
  afterAll(() => {});
  afterEach(async () => {
    ddbMock.reset();
  ...
  it('Should send a valid account ID', async function() {
    ...
    const result = await app.Handler({ body: '{"accountId":"1234"}' }, {});
